...
function setCommentReplyHandler(): void {
    $('.post-comments').on('click', '[data-comment-form-trigger]', replyLinkClickEH)
}

function replyLinkClickEH(e: JQueryEventObject): void {
    e.preventDefault();
    ...
    commentForm.show();
     ...
     commentForm.on('submit', function (e: JQueryEventObject) {
        e.preventDefault();
        e.stopPropagation();
        var targetCommentForm = $(e.target);
        if (targetCommentForm.valid()) {
...
function likePostSubmitHandler(e: JQueryEventObject) {
    e.preventDefault();
...
function dislikePostSubmitHandler(e: JQueryEventObject) {
    e.preventDefault();
    var submitBtn = $('button[type="submit"]', e.target).prop('disabled', true);
    var url = "/api" + e.target.getAttribute('action');
